...
if img_file:
    ...
    with st.spinner("鉄道電気設備の専門AIが解析中..."):
        try:
            model = genai.GenerativeModel('gemini-2.5-flash-lite')
            
            # 専門的な判定用プロンプト
            prompt = f"""
あなたは鉄道電気設備の保守点検エキスパートです。
提出された写真を以下の視点で分析し、結果を報告してください。

【確認カテゴリ】: {category}

【指示】
1. 異常の有無を専門的な用語（{category}に関連する用語）を用いて具体的に判定してください。
2. 異常がある場合は、その箇所と程度を推測してください。
3. 最後に、この写真にふさわしい「20文字以内のタイトル」を1行だけで出力してください。

【タイトル付与の留意事項】
- 写真には電気設備が写っているため、写実的で具体的なタイトルにすること。
